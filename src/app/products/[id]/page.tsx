...
const Detailpageshop: React.FC<Params> = ({ params }) => {
        ...
        const fetchProduct = async () => {
            try {
                ...
                const response = await fetch(
                    `https://ui-ux-hackathon-xi.vercel.app/api/products/${unwrappedParams.id}`,
                    { cache: 'no-store' }
                );
                if (!response.ok) {
                    throw new Error("Failed to fetch product data");
                }
                const productData: Data = await response.json();
